#!/usr/bin/env python3
"""Smoke test - upload image, poll status, get result."""
import sys
import time
import requests
import os

API_URL = os.environ.get("API_URL", "http://localhost:8000")
TIMEOUT = int(os.environ.get("SMOKE_TIMEOUT", "60"))
POLL_INTERVAL = 2

#1x1 red pixel JPEG
TINY_JPEG = bytes([
    
])


def main():
    print(f"Smoke test against {API_URL}")

    try:
        r = requests.get(f"{API_URL}/health", timeout=5)
        if r.status_code != 200:
            print(f"FAIL: health check returned {r.status_code}")
            return 1
    except requests.RequestException as e:
        print(f"FAIL: cannot reach API - {e}")
        return 1

    print("Health check OK")

    files = {"inputFile": ("test.jpg", TINY_JPEG, "image/jpeg")}
    try:
        r = requests.post(f"{API_URL}/", files=files, timeout=30)
    except requests.RequestException as e:
        print(f"FAIL: upload request failed - {e}")
        return 1

    if r.status_code not in (200, 202):
        print(f"FAIL: upload returned {r.status_code}: {r.text}")
        return 1

    data = r.json()
    job_id = data.get("job_id")
    if not job_id:
        print(f"FAIL: no job_id in response: {data}")
        return 1

    print(f"Upload OK, job_id={job_id}, status={data.get('status')}")

    if data.get("status") == "done":
        print(f"Result: {data.get('result')}")
        print("PASS")
        return 0

    deadline = time.time() + TIMEOUT
    while time.time() < deadline:
        time.sleep(POLL_INTERVAL)
        try:
            r = requests.get(f"{API_URL}/status/{job_id}", timeout=10)
            data = r.json()
            print(f"Status: {data.get('status')}")
            if data.get("status") == "done":
                print(f"Result: {data.get('result')}")
                print("PASS")
                return 0
        except requests.RequestException as e:
            print(f"Poll error: {e}")

    print(f"FAIL: timeout waiting for result after {TIMEOUT}s")
    return 1


if __name__ == "__main__":
    sys.exit(main())
